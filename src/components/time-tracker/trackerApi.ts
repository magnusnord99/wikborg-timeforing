...
import { toLocalDayRangeIso } from '../time-utils'
...
async function fetchEntriesBetween(startIso: string, endIso: string, ascending: boolean) {
  ...
  while (true) {
    ...
    if (page.length < PAGE_SIZE) return { data: entries, error: null }
    from += PAGE_SIZE
  }
}
...
export async function fetchEntriesForDate(selectedDate: string) {
  const { start, end } = toLocalDayRangeIso(selectedDate)
...
export async function fetchEntriesForRange(startDate: string, endDate: string) {
  const { start } = toLocalDayRangeIso(startDate)
  const { end } = toLocalDayRangeIso(endDate)
