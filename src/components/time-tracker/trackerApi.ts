...
import { getLocalDateEndExclusiveIso, getLocalDateStartIso } from '../time-utils'
...
export async function fetchEntriesForDate(selectedDate: string) {
  return fetchEntriesBetween(
    getLocalDateStartIso(selectedDate),
    getLocalDateEndExclusiveIso(selectedDate),
    false,
  )
}
...
export async function fetchEntriesForRange(startDate: string, endDate: string) {
  return fetchEntriesBetween(
    getLocalDateStartIso(startDate),
    getLocalDateEndExclusiveIso(endDate),
    true,
  )
}

async function fetchEntriesBetween(startIso: string, endExclusiveIso: string, ascending: boolean) {
